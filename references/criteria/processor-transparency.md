...
1. Tokenize processor names from **three** corpora: (1) the privacy-policy prose, (2) the cookie policy, and (3) `legalPageContent.cmpVendorList` (the JS-rendered CMP per-vendor list surfaced in the brief as "CMP vendor list") — many named processors appear only in the CMP vendor list, so searching the prose alone over-reports "undisclosed"
