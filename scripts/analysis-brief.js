...
const vcm = details.vendorConsentModes || {};
const ms = vcm.microsoft || {};
const amz = vcm.amazon || {};
ln(`Microsoft UET consent mode: ${ms.detected ? `detected (tag=${!!ms.tagPresent}, consentCalls=${!!ms.consentMode})` : "not detected"}`);
ln(`Amazon TAM/ats: ${amz.detected ? `detected (tam=${!!amz.tam}, ats=${!!amz.ats}, tag=${!!amz.tagPresent})` : "not detected"}`);
